...
DEFAULT_CAM_POSES = {
    "side": [
        [ 0.99445658, -0.02257474, -0.10269611, -0.46575977],
        [-0.09759366, -0.56168058, -0.82157799,  0.04976504],
        [-0.03913550,  0.82704613, -0.56077010,  1.14264427],
        [ 0.00000000,  0.00000000,  0.00000000,  1.00000000]
    ],
    "head": [
        [ 0.02882777, -0.99935971, -0.02119273, -0.03698096],
        [-0.98584415, -0.03192837,  0.16459617,  0.74006182],
        [-0.16516743,  0.01614779, -0.98613334,  0.97828253],
        [ 0.00000000,  0.00000000,  0.00000000,  1.00000000]
    ]
}
...
class FlexivRobot():
    ...
    def motor_features(self) -> dict:
        ...
        for cam_key, cam in self.cameras.items():
            ex_key = f"observation.extrinsics.{cam_key}"
            motor_ft[ex_key] = {
                "dtype": "float32",
                "shape": (16,),  # (4, 4) matrix flattened
            }
            in_key = f"observation.intrinsics.{cam_key}"
            motor_ft[in_key] = {
                "dtype": "float32",
                "shape": (9,),  # (3, 3) matrix flattened
            }
        return motor_ft
    ...
    def teleop_step(
        ...
        action_valid = (action is not None) and (
            gripper_width is not None) and not outofbound
        ...
        for name in self.cameras:
            ...
            if name in depth_maps:
                obs_dict[f"observation.depth.{name}"] = Image.fromarray(depth_maps[name].numpy(), mode="I;16")

            if name == "wrist":
                obs_dict[f"observation.extrinsics.{name}"] = np.linalg.inv(cam_pose).flatten()
            else:
                obs_dict[f"observation.extrinsics.{name}"] = np.array(DEFAULT_CAM_POSES[name]).flatten()

            obs_dict[f"observation.intrinsics.{name}"] = self.cameras[name].intrinsics.flatten()
    ...
    def capture_observation(self) -> dict:
        ...
        for name in self.cameras:
            ...
            if name in depth_maps:
                ...
                print("Depth Dtype: ", depth_maps[name].dtype)
                print("Depth Shape: ", depth_maps[name].dtype)
