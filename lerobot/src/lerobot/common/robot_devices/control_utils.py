    ...
    def on_press(key):
        try:
            if key == keyboard.Key.right:
                logger.info("Right arrow key pressed. Exiting loop...")
                ...
                logger.info("Escape key pressed. Stopping data recording...")
...
def control_loop(
    ...
    while timestamp < control_time_s:
        ...
        if quest_streamer is not None:
            if quest_streamer.camera_key not in observation:
                raise KeyError(
                    f"Quest stream camera key {quest_streamer.camera_key!r} is missing from observation."
                )
            quest_streamer.publish_frame(observation[quest_streamer.camera_key])
        ...
        if display_cameras and not is_headless():
            ...
            positions = {
                ...
                "observation.images.front": (0, 0),        # bottom right
                "observation.images.aside": (1400, 0),        # bottom right
            }
    ...
    logger.info('End of the Episode')
