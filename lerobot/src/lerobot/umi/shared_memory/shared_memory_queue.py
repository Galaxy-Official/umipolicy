...
from lerobot.umi.shared_memory.shared_memory_util import ArraySpec, SharedAtomicCounter
from lerobot.umi.shared_memory.shared_ndarray import SharedNDArray
...
class SharedMemoryQueue:
    ...
    def _clear_k(self, k):
        current_read_count = self.read_counter.load()
        new_read_count = current_read_count + k
        write_count = self.write_counter.load()
        if new_read_count > write_count:
            raise ValueError("read_count > write_count, cannot clear")
