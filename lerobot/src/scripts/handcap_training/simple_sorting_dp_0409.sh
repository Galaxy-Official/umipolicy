...
OUTPUT_DIR="outputs/train/simple_sorting_0426_resnet50"
...
accelerate launch --multi_gpu --num_processes=2 --num_machines=1 --main_process_port=29502 --mixed_precision=bf16 --dynamo_backend=inductor -m lerobot.scripts.lerobot_train \
  --dataset.repo_id=lihongcs/simple_sorting_handcap \
  --dataset.root=Data/simple_sorting_425train \
  ...
  --job_name=dp_simple_sorting_resnet50_handcap \
  --policy.device=cuda \
  --wandb.enable=true \
  --use_handcap=true \
  --steps=200000 \
