...
class LeRobotHandcapWristPointDataConfig(DataConfigFactory):
    """Handcap LeRobot data config for wrist RGB + phone RGBD generated pointcloud."""
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # 注意：info.json 里没有 observation.pointcloud.phone
        # 所以这里 repack phone RGB + phone_depth，交给 HandcapInputs 生成 pointcloud
        repack_structure = {
        ...
        if getattr(model_config, "use_point", False):
            repack_structure["observation/pointcloud"] = "observation.pointcloud.phone"

        repack_transform = _transforms.Group(
            inputs=[
                _transforms.HandcapRepackTransform(repack_structure)
            ]
        )
        ...
        data_transforms = _transforms.Group(
            inputs=[
                handcap_policy.HandcapInputs(
                    ...
                    force_dim=force_dim,
                    use_point=getattr(model_config, "use_point", False),
                    point_num_points=getattr(model_config, "point_num_points", 1024),
                    point_dim=getattr(model_config, "point_dim", 3),
                    point_key=getattr(model_config, "pointcloud_keys", ("phone_pointcloud",))[0],
                )
            ],
            outputs=[
        ...
        delta_action_mask = _transforms.make_bool_mask(
            action_base_dim,
            -(model_config.action_dim - action_base_dim),
        )
...
def get_handcap_configs():
    ...
    return [
        TrainConfig(
            name="pi05_erase_board_wrist_point_debug",
            model=pi0_config.Pi0Config(
                pi05=True,
                use_tactile=False,
                tactile_pretrained_ckpt="",
                use_point=True,
                pointcloud_keys=("phone_pointcloud",),
                point_num_points=1024,
                point_dim=3,
                point_token_len=8,
                point_encoder_type="pointnet",
                camera_keys=("wrist_0_rgb",),
            ),
            data=LeRobotHandcapWristPointDataConfig(
            ...
            weight_loader=weight_loaders.CheckpointWeightLoader(
                "/inspire/hdd/project/robot-reasoning/xuyue-p-xuyue/lihong_workspace/lihong/umipolicy/openpi/ckpt/pi05_base/params"
            ),
        ...
        TrainConfig(
            name="pi05_430_towel_hanging_debug",
            model=pi0_config.Pi0Config(
                pi05=True,
                use_tactile=False,
                tactile_pretrained_ckpt="",
        ...
        TrainConfig(
            name="pi05_430_towel_hanging_tactile_debug",
            model=pi0_config.Pi0Config(
                pi05=True,
                use_tactile=True,
                tactile_pretrained_ckpt="/inspire/hdd/project/robot-reasoning/xuyue-p-xuyue/lihong_workspace/lihong/umipolicy/openpi/ckpt/pretrained_tactile_encoder.pt",
