...
class KPSTExecutor(object):
    ...
    def find_corresponding_3d_point(self, gripper_2d_pos, pcd_scene, depth_image):
        ...
        x = int(round(gripper_2d_pos[0]))  # 行索引对应 y
        y = int(round(gripper_2d_pos[1]))  # 列索引对应 x
        depth_value = depth_image[x, y]
        # print("gripper_2d_pos",gripper_2d_pos)
    ...
    def crop_point_cloud_by_depth(self, pcd, max_depth=0.8):
        ...
        Args:
            ...
            max_depth: float, maximum depth in meters (Z-coordinate)

        Returns:
            cropped_pcd: open3d.geometry.PointCloud, cropped point cloud
        """
        pcd_pos = np.asarray(pcd.points)
    ...
    def visualize_point_cloud(self, pcd, title="Point Cloud Visualization"):
        ...
        try:
            ...
            # Fallback: Save point cloud to file for later inspection
            try:
                import os
    ...
    def geometric_generation(self, rgb_image, depth_image, gripper_pos, mask=None, area_bound=None, enable_visualization=True):
        # pdb.set_trace()
        ...
        # Crop point cloud - remove points with depth > 0.8 meters
        original_points = len(pcd_scene.points)
        pcd_scene = self.crop_point_cloud_by_depth(pcd_scene, max_depth=0.8)
        cropped_points = len(pcd_scene.points)

        if cropped_points < original_points:
            print(f"Cropped point cloud: {original_points} -> {cropped_points} points (removed {original_points - cropped_points} points with depth > 0.8m)")
            # Visualize cropped point cloud (only if enabled)
            if enable_visualization:
                self.visualize_point_cloud(pcd_scene, title=f"Cropped Point Cloud (depth < 0.8m) - {cropped_points} points")
            else:
                print("Visualization disabled for headless environment.")
    ...
    def get_kpst_prediction(self, pcd, kps_3d, return_np=False):
        ...
        data = self.data_transform(data_orig)
    ...
    def get_motion_planning(self, kpst, weights, plan_step=1, commit=''):
        ...
        for i in range(plan_step):
            ...
            T[2,3] = t[0][2]
            #print("T",T)
            motion_plan_SE3.append(T)
            motion_plan.append((R,t, success))
        return motion_plan,motion_plan_SE3
    ...
    def kpst_motion_execusion(self, rgb_image, depth_image, gripper_pos,
        ...
        mask = None
        pcd, gripper_3d_pos, pcd_org = self.geometric_generation(rgb_image, depth_image, gripper_pos, mask=mask, area_bound=area_bound, enable_visualization=enable_visualization)
        ...
        kpst, input_data = self.get_kpst_prediction(pcd, kps_3d, return_np=False)   # (M, Q, T, 3), cuda
        weights = torch.Tensor(weights / np.sum(weights)).to(self.device)
        motion_plan, motion_plan_SE3 = self.get_motion_planning(kpst, weights, plan_step=plan_step, commit=commit)
        if vis_dir is not None:
            vis_kpst = kpst.detach().cpu().numpy()
            vis_result = self.save_exec_data_to_dir(vis_kpst, gripper_3d_pos, motion_plan, motion_plan_SE3,pcd_org, vis_dir=vis_dir, commit=commit)
        else:
            vis_result = None
        return motion_plan, vis_result
    
    
    def save_exec_data_to_dir(self, vis_kpst, gripper_3d_pos, motion_plan,motion_plan_SE3, pcd_org, vis_dir, commit=''):
        ...
        result = {
            ...
            'motion_plan_SE3':motion_plan_SE3,
        }
        if len(commit) > 0:
            commit = '_' + commit
        # desc_id = self.desc.replace(' ', '_')
        save_fp = os.path.join(vis_dir, f'pred_gflow.pkl')
...
def exec_kpst_affordance_from_input_dir(exec_model, input_dir, desc,
    ...
    # Read depth from H5 file - use first frame (00000_depth.png)
    depth_path = os.path.join(input_dir, 'depth.h5')
    ...
    try:
        with h5py.File(depth_path, 'r') as f:
            # Use the first frame dataset (00000_depth.png)
            first_frame_key = '00000_depth.png'
            if first_frame_key not in f:
                print(f"Error: '{first_frame_key}' dataset not found in {depth_path}")
                print(f"Available datasets: {list(f.keys())}")
                exit(1)

            numpy_depth = f[first_frame_key][:]  # Get the first frame depth data
            print(f"Loaded depth from {first_frame_key}: shape={numpy_depth.shape}, dtype={numpy_depth.dtype}")
    ...
    exec_model.kpst_motion_execusion(rgb_image, depth_image, gripper_2d_pos, policy_radius=policy_radius, policy_env=policy_env, policy_kps_max=policy_kps_max,
                                     robot_anchor=robot_anchor, plan_step=plan_step, commit=commit, vis_dir=vis_dir, area_bound=area_bound, enable_visualization=enable_visualization)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_dir', type=str, default='../pred_keypoints/capture_0301/box/view_000_pose_000')
    ...
    parser.add_argument('--enable_visualization', action='store_true', help='enable Open3D visualization (disable for headless environments)')
    parser.add_argument('-p', '--pretrained_path', type=str,
                        default='log/kpst_hoi4d/ScaleGFlow-L/checkpoint/ckpt_best_train_scalegflow_l.pth')
    ...
    rgb_path = os.path.join(input_dir, 'rgb.jpg')
    if not os.path.exists(rgb_path):
        print(f"Error: RGB file not found at {rgb_path}")
    ...
    if rgb is None:
        print(f"Error: Failed to read RGB image from {rgb_path}")
        exit(1)

    H, W = rgb.shape[0], rgb.shape[1]

    # Set fixed camera intrinsics
    K = np.array([
        [910.12329102,   0. ,        649.51123047],
        [  0.,         908.83721924, 369.29821777],
        [  0. ,          0.,           1.        ]
    ])
    exec_model.set_camera(K, H=H, W=W)
    ...
    exec_kpst_affordance_from_input_dir(exec_model, input_dir, desc, policy_radius=0.1, gripper_2d_pos=None, robot_anchor=None, vis_dir=vis_dir, enable_visualization=args.enable_visualization)
    # pdb.set_trace()
...
# python aff_exec_modified_crop.py --desc open_Box
# python aff_exec_modified_crop.py --desc open_Box --enable_visualization  # Enable visualization
# python aff_exec_modified_crop.py --desc open_Box  # Disable visualization (default for headless)