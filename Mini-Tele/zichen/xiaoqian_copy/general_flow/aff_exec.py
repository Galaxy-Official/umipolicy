...
class KPSTExecutor(object):
    ...
    def kpst_motion_execusion(self, rgb_image, depth_image, gripper_pos, 
        ...
        if vis_dir is not None:
            ...
            pickle.dump(input_data, open(os.path.join(vis_dir, f'input_data.pkl'), "wb"))
        else:
            vis_result = None
...
if __name__ == "__main__":
    ...
    os.makedirs('demo/output_tmp', exist_ok=True)
    args, opts = parser.parse_known_args()
    args.cfg = '/'.join(args.pretrained_path.split('/')[:-2] + ['cfg.yaml'])
    args.save_dir = f"demo/output_tmp/{args.input_dir.split('/')[-1]}"
