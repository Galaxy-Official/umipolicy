...
if __name__ == "__main__":
    ...
    # 视频保存路径
    calib_group = "capture_1113/document_box"
    view_id = 0
    pose_id = 0           
    ...
    while True:
        ...
        global_key = f"view_{str(view_id).zfill(3)}_pose_{str(pose_id).zfill(3)}"
        # root_path = f"calib_out/{calib_group}/{global_key}"
        root_path = f"pred_keypoints/{calib_group}/{global_key}"
        ...
        # depth_0 = wr_depth[str(id).zfill(5) + "_depth.png"]
        # pickle.dump(depth_0, open(f"{root_path}/depth.pkl", "wb"))
