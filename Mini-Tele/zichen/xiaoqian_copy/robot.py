...
class Flexiv():
    ...
    def __init__(self, robot_ip = ROBOT_IP, local_ip = LOCAL_IP, move_home=True, init_offset=None, init_qpos=None, device="cuda"):
        ...
        # self.gripper.move(0.001, 0.1, 5) # close gripper
        self.gripper.move(0.8, 0.1, 5)
        # if move_home:
        #     self.move_to_home()
    ...
    def send_tcp_pose(self, tcp):
        ...
        flange_pose = np.array([*tcp[:3], tcp[4], tcp[5], tcp[6], tcp[3]]) # wxyz -> xyzw
        flange_pose[2] += 0.14
        next_joints = self.sim_rizon.calc_ik(flange_pose, is_tcp=False)  # pppqqqq -> q
        # print("next_joints",next_joints)
        self.send_joint_position(next_joints)

    def set_gripper_width(self, width):
        self.gripper.move(width, 0.1, 5)
        time.sleep(0.05)
...
if __name__ == "__main__":
    ...
    HOME_POSE = np.array(
            json.load(
            open("pred_keypoints/capture_0301/box/view_000_pose_000/robot_pose.json", "r")))
    ...
    # time.sleep(5)
    # robot.send_gripper_state(0.001 ,0.1, 20)
    # print("pose",robot.get_joint_positions())
    print(robot.readCamPose())
    # time.sleep(1.0)
    # robot.robot.setMode(robot.mode.NRT_PRIMITIVE_EXECUTION)
    # robot.robot.executePrimitive("Home(jntVelScale=5)")
    # while robot.robot.isBusy():
    #     time.sleep(1)