...
if __name__ == '__main__':
    ...
    for rank, selected_grasp_idx in enumerate(selected_grasp_idxs):
        ...
        os.makedirs(f"{args.root_dir}/{args.global_key}/init_info/init_grasp_all", exist_ok=True)
        temp_response_path = f"{args.root_dir}/{args.global_key}/init_info/init_grasp_all/{str(rank).zfill(3)}.npz"
        ...
        os.makedirs(f"{args.root_dir}/{args.global_key}/init_info/init_grasp_all_orig", exist_ok=True)
        pickle.dump(gg_grasp[selected_grasp_idx:selected_grasp_idx+1], 
                    open(f"{args.root_dir}/{args.global_key}/init_info/init_grasp_all_orig/{str(rank).zfill(3)}.pkl", "wb"))
