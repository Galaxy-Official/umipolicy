...
if __name__ == '__main__':
    ...
    # Define start and end poses for demonstration
    # start_pose = np.array([0.6, 0.0, 0.3, 1, 0, 0, 0])  # x, y, z, w, x, y, z
    # end_pose = np.array([0.6, 0.2, 0.4, 1, 0, 0, 0])    # Different position, same orientation

    # start_pose = np.array([0.038846,-0.463577,0.170337,1.689539,-0.098537,0.684736,0.199307]) # 50
    # end_pose = np.array([-0.255202,-0.818133,0.110498,0.980329,0.656995,0.203076,-0.274998]) # 200

    # fixed grasp
#     start_pose = np.array([0.77089993,  0.0346396,   0.26912117, -0.02578649 , 0.9979465,   0.01001209,
#   0.05777195]) # 50
#     end_pose = np.array([0.7807473,  -0.07870434,  0.42852162,  0.02692586,  0.91849265,  0.38814242,
#   0.07065211]) # 200

#     start_pose = np.array([0.65134619,  0.01040765 , 0.2531466 ,  0.13896455 , 0.73400466 , -0.08502046,
#   0.65931598])
#     end_pose = np.array([0.66096311, -0.07755542,  0.40112907,  0.40171149,  0.70183246,  0.18347719,
#   0.55892326])
    flange_pose_waypoints = np.load("collect_data/data/kp/2025-10-26_19:29:53/flange_pose_waypoints/003.npy")
    ...
    # print("\n=== Testing Custom RRT Planner ===")
    # rrt_planner = RRTPlanner(robot, step_size=0.05, max_iterations=500)
    # success, rrt_path, message = rrt_planner.plan_and_validate_movement(start_pose, end_pose, visualize_path=True)

    # print(f"RRT Planning result: {message}")

    # if success and rrt_path is not None:
    #     print(f"RRT path planning successful! Found path with {len(rrt_path)} waypoints")

    #     # Execute the RRT path
    #     input("Press Enter to execute RRT planned path...")
    #     rrt_planner.execute_path(rrt_path, step_duration=0.1)

    #     # Test path validation
    #     print("\n=== Path Validation Test ===")
    #     is_valid, collision_idx = robot.validate_path(rrt_path)
    #     print(f"RRT Path validation: {'Valid' if is_valid else 'Invalid'}")
    #     if not is_valid:
    #         print(f"Collision detected at waypoint {collision_idx}")
    # else:
    #     print("RRT path planning failed!")

    # # Test collision checking
    # print("\n=== Collision Checking Test ===")
    # current_joints = robot.get_joints()
    # is_collision = robot.check_collision(current_joints)
    # print(f"Current configuration collision check: {'In collision' if is_collision else 'Collision free'}")

    # # Test interpolation
    # print("\n=== Joint Interpolation Test ===")
    # start_joints = robot.calc_ik(start_pose)
    # end_joints = robot.calc_ik(end_pose)
    # interpolated_path = robot.interpolate_joints(start_joints, end_joints, num_waypoints=10)
    # print(f"Generated interpolated path with {len(interpolated_path)} waypoints")

    # # Visualize interpolated path
    # print("Visualizing interpolated path...")
    # for i, joints in enumerate(interpolated_path):
    #     robot.set_joints(joints)
    #     p.stepSimulation()
    #     tcp_pos = robot.get_tcp_catersian()[:3]
    #     robot.visualize_point(tcp_pos, (0, 0, 1), 4)  # Blue for interpolated points
    #     if i % 2 == 0:  # Add text every other point to avoid clutter
    #         p.addUserDebugText(f"I{i}", tcp_pos, textColorRGB=(0, 0, 1), textSize=1.0)
