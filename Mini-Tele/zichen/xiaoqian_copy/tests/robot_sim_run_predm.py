...
if __name__ == '__main__':
    ...
    ## for door
    pred_keypoints = json.load(open(f"{args.root_dir}/predm_keypoints.json", "r"))
    pred_part_points = pred_selected_affordable_position = np.array(pred_keypoints["kp_contact1"])
    point1 = np.array(pred_keypoints["kp_axis1"])
    point2 = np.array(pred_keypoints["kp_axis2"])
    contact_points = np.array([pred_keypoints["kp_contact1"], 
                               pred_keypoints["kp_contact2"],
                               pred_keypoints["kp_contact3"],
                               pred_keypoints["kp_contact4"]])

    ## for box
    # point1 = gt_point1 + np.random.normal(0, 0.01, 3)
    # point2 = gt_point2 + np.random.normal(0, 0.01, 3)
    # pred_part_points = pred_selected_affordable_position = gt_pred_selected_affordable_position + np.random.normal(0, 0.02, 3)
    # contact_points = np.array([pred_part_points,
    #                             pred_part_points + np.random.normal(0, 0.005, 3),
    #                            pred_part_points + np.random.normal(0, 0.005, 3),
    #                            pred_part_points + np.random.normal(0, 0.005, 3),
    #                            pred_part_points + np.random.normal(0, 0.005, 3)])
    # pred_keypoints_new = {
    ...
    # }
    # # pickle.dump(pred_keypoints_new, open(f"{args.root_dir}/pred_m_keypoints_new.pkl", "wb"))
    ...
    for contact_point in contact_points:
        print(contact_point)
        mesh_sphere = draw_point(contact_point, [0, 0, 1], radius=radius)
        vis.add_geometry(mesh_sphere)
    # mesh_sphere = draw_point(pred_part_points, [0, 0, 1], radius=radius)
    # vis.add_geometry(mesh_sphere)
