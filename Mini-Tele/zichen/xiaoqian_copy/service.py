...
def config_parse() -> configargparse.Namespace:
    ...
    parser.add_argument('--gsnet_weight_path', type=str, default='/home/rhos/xiaoyang/anygrasp_sdk/grasp_detection/log/checkpoint_detection.tar', help='the path to graspnet weight')
...
# [Open3D INFO] Adding point #69168 (0.07, -0.16, 0.47) to selection.
# [Open3D INFO] Adding point #148349 (-0.13, -0.13, 0.47) to selection.
# [Open3D INFO] Adding point #578885 (-0.01, 0.05, 0.45) to selection.
    ...
    pred_selected_affordable_position = np.array([0.09, 0.00, 0.45])
    # point1 = np.array([0.19, 0.13, 0.57])
    # point2 = np.array([0.22, -0.14, 0.58])

    point1 = np.array([-0.07, -0.04, 0.63])
    point2 = np.array([-0.07, 0.16, 0.42])

    pred_selected_joint_direction,pred_selected_joint_base = calculate_rotation_axis(point1,point2)

    args = config_parse()
    setup_seed(args.seed)
    temp_request_path = '/home/rhos/xiaoyang/temp_data/observation.npz'
    ...
    while True:
        ...
        if args.graspnet:
            ...
            # selected_grasp_idx = np.argsort(grasp_affordances)[::-1][0]
            
            # vis_idx = np.argsort(grasp_affordances)[::-1][0:1]
            # grippers_o3d = gg_grasp[vis_idx].to_open3d_geometry_list()
            # frame_o3d = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
            # o3d.visualization.draw_geometries([*grippers_o3d, pcd_o3d, frame_o3d])
        ...
        # print(os.path.isfile(temp_response_path), os.access(temp_response_path, os.R_OK))
        while not (os.path.isfile(temp_response_path) and os.access(temp_response_path, os.R_OK)):
            time.sleep(0.1)
        serviced = np.array(True)
        np.save(temp_flag_path, serviced)
        ...
        while True:
            # print(os.path.isfile(temp_flag_path) and os.access(temp_flag_path, os.R_OK))
            while not (os.path.isfile(temp_flag_path) and os.access(temp_flag_path, os.R_OK)):
                time.sleep(0.1)
            serviced = np.load(temp_flag_path).item()
            if not serviced:
